	...
	dbc.Container([
		...
		dbc.Row([
			dbc.Col([
				dbc.Nav([
					dbc.NavItem(
						dcc.Dropdown(assets, value='KGH', searchable=False, clearable=False, id='asset')
					),
					dbc.NavItem(
					...
					dbc.NavItem(
						dbc.DropdownMenu(label='Oscillator 1', children=[
							dcc.RadioItems(['Volume', 'MACD', 'RSI', 'Stochastic', 'SMA Ratios', 'EMA Ratios', 'Williams %R', 'CCI', 'Aroon', 'EMD Components', 'Close % Change'], 'EMD Components' , id='oscillator1', labelStyle={'display': 'block'}),
						]),
					),
					dbc.NavItem(
						dbc.DropdownMenu(label='Oscillator 2', children=[
							dcc.RadioItems(['Volume', 'MACD', 'RSI', 'Stochastic', 'SMA Ratios', 'EMA Ratios', 'Williams %R', 'CCI', 'Aroon', 'EMD Components', 'Close % Change'], 'Close % Change' , id='oscillator2', labelStyle={'display': 'block'}),
						]),
					),
					dbc.NavItem(
			...
			if 'EMD-LSTM' in predictions:
				for component in emd_components:
					if component == list_emd_components('KGH')[-1]:
						...
						fig.update_yaxes(title_text='EMD Components', row=row_number)
		elif oscillator_number == 'Close % Change':
			fig.add_trace(go.Scatter(x=df['Date'], y=df['Actual Close % Change'], name='Actual Close % Change'), row=row_number, col=1)
			if 'EMD-LSTM' in predictions:
				fig.add_trace(go.Scatter(x=df['Date'], y=df['Predicted Close % Change'], name='Predicted Close % Change'), row=row_number, col=1)
				fig.update_yaxes(title_text='Close % Change', row=row_number)
